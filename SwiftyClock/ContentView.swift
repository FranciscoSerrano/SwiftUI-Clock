...
struct ContentView: View {
    var body: some View {
        TimelineView(.animation(minimumInterval: 1 / 20)) { timeline in
            Canvas { context, size in
                ...
                drawMilitaryHours(in: context, radius: radius / 1.5)
    ...
    func drawMilitaryHours(in context: GraphicsContext, radius: Double) {
        let textSpace = CGSize(width: 200, height: 200)
        let textSize = radius / 5
        let textOffset = radius * 0.75

        for i in 13...24 {
            var contextCopy = context
            let text = Text(String(i)).font(.system(size: textSize))
            let resolvedText = contextCopy.resolve(text)
